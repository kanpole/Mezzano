...
(defpackage :mezzano.disassemble.x86-64
  (:use :cl)
           ...
           (dolist (operand (inst-operands instruction))
             ...
             (cond ((typep operand 'effective-address)
                    (cond ((eql (ea-base operand) :rip)
                           (let* ((address (logand (sys.int::lisp-object-address (dis:context-function context)) -16))
                                  (target (+ (dis:inst-offset instruction)
                                             (dis:inst-size instruction)
                                             (ea-disp operand)))
                             ...
                             (cond
                               ...
                               (t
                                (cond ((and (not (logtest target #b111))
                                            (<= 0 pool-index)
                                            (not (sys.int::function-reference-p (dis:context-function context)))
                                            (< pool-index (sys.int::function-pool-size (dis:context-function context))))
                                       (let ((pool-object (sys.int::function-pool-object (dis:context-function context) pool-index)))
                                         (push
                                          (let ((*print-lines* 1)
                                                (*print-length* 2)
                                                (*print-level* 2))
                                      ...
                                      ((and (eql (inst-opcode instruction) 'sys.lap-x86:lea64)
                                       ...
                                       (push (format nil "'~S" (dis:context-function context)) annotations)))
...
(defclass x86-64-instruction (dis:instruction)
...
(defun disassemble-modr/m (context info)
  ...
  (let* ((modr/m (dis:consume-ub8 context))
         (rex-b (rex-b info))
         (rex-x (rex-x info))
     ...
     (ecase (ldb +modr/m-mod+ modr/m)
       (#b00
        (case (ldb +modr/m-r/m+ modr/m)
          (#b100
           (let ((sib (dis:consume-ub8 context)))
             (cond ((eql (ldb +sib-base+ sib) 5)
                    ;; No base, disp32 follows.
                    (let ((disp32 (dis:consume-sb32/le context)))
             ...
             (cond ((and (not rex-x) (eql (ldb +sib-index+ sib) 4))
                   ...
                   (t
                    (make-instance 'effective-address
                                   ...
                                   :segment (getf info :segment))))))
          (t
           (let ((disp8 (dis:consume-sb8 context)))
             (make-instance 'effective-address
                            :base (if asize
                                      (decode-gpr32 (ldb +modr/m-r/m+ modr/m) rex-b)
                                      (decode-gpr64 (ldb +modr/m-r/m+ modr/m) rex-b))
       ...
       (#b10
        (case (ldb +modr/m-r/m+ modr/m)
          (#b100
           (let ((sib (dis:consume-ub8 context))
                 (disp32 (dis:consume-sb32/le context)))
             (cond ((and (not rex-x) (eql (ldb +sib-index+ sib) 4))
                    (make-instance 'effective-address
                                   :base (if asize
                                             (decode-gpr32 (ldb +sib-base+ sib) rex-b)
...
(defun decode-ib (context info opcode)
  (declare (ignore info))
  (make-instruction opcode
                    (dis:consume-sb8 context)))

(defun decode-iz (context info opcode)
  (declare (ignore info))
  (make-instruction opcode
                    (dis:consume-sb32/le context)))
...
(define-modr/m-decoder decode-ev-gv-ib (context info opcode r/m reg)
  (let ((imm (dis:consume-sb8 context)))
...
(define-modr/m-decoder decode-gv-ev-iz (context info opcode r/m reg)
  (let ((imm (dis:consume-sb32/le context)))
    (make-instruction opcode reg r/m imm)))
    ...
    (multiple-value-bind (opcode16 opcode32 opcode64 has-imm)
      ...
      (ecase (operand-size info)
        (64
         (and opcode64
              (if has-imm
                  (make-instruction opcode64 (decode-gpr64-or-mem r/m (rex-b info)) (dis:consume-sb32/le context))
                  (make-instruction opcode64 (decode-gpr64-or-mem r/m (rex-b info))))))
        (32
         (and opcode32
              (if has-imm
                  (make-instruction opcode32 (decode-gpr32-or-mem r/m (rex-b info)) (dis:consume-sb32/le context))
                  (make-instruction opcode32 (decode-gpr32-or-mem r/m (rex-b info))))))
        (16
         (and opcode16
              (if has-imm
                  (make-instruction opcode16 (decode-gpr16-or-mem r/m (rex-b info)) (dis:consume-sb16/le context))
...
(defun decode-simd-shift (context info insts)
  (multiple-value-bind (reg r/m)
      (disassemble-modr/m context info)
    (let ((imm (dis:consume-ub8 context)))
...
(defun decode-al-ob (context info opcode)
  ...
  (make-instruction opcode
                    :al
                    (make-instance 'effective-address
                                   :disp (dis:consume-sb64/le context))))
...
(defun disassemble-one-instruction-1 (context)
  (let* ((byte (dis:consume-ub8 context))
    ...
    (loop
       (case byte
         ...
         (t
          (return)))
       (setf byte (dis:consume-ub8 context)))
...
(defmethod dis:disassemble-one-instruction ((context x86-64-disassembler-context))
           ...
           (let ((inst (disassemble-one-instruction-1 context)))
             (cond (inst
                    ...
                    (when (and (eql (inst-opcode inst) 'sys.lap-x86:jmp)
                      ...
                      (label context (dis:context-code-offset context) :createp t)
                      (setf (decoding-jump-table-p context) (dis:context-code-offset context)))
                    (setf (slot-value inst 'dis::%offset) start
                          (slot-value inst '%size) (- (dis:context-code-offset context) start))
                    ;; Update labels.
                    (dolist (operand (inst-operands inst))
                      (when (and (typep operand 'effective-address)
                                 (eql (ea-base operand) :rip)
                                 (<= code-start
                                     (+ (dis:context-code-offset context) (ea-disp operand))
                                     (1- code-end)))
                        (dis:label context
                    ...
                    inst)
                   (t
                    (setf (slot-value context 'dis::%offset) (1+ start))
                    nil)))))))
